...
>
> ![知识星球推广](images/%E7%9F%A5%E8%AF%86%E6%98%9F%E7%90%83%E6%8E%A8%E5%B9%BF-20221121234058789-20221125105452668.jpeg)
>
...
>
> ![公众号-爱编程的mast](images/%E5%85%AC%E4%BC%97%E5%8F%B7-%E7%88%B1%E7%BC%96%E7%A8%8B%E7%9A%84mast-20221125105456624.png)

# Mybatis学习笔记



## ![mybatis-logo](images/mybatis-logo.jpeg)简介
...
> 在Maven项目中的pom.xml文件中导入mybatis依赖
...
~~~mermaid
graph LR
下载Mybatis的Jar包 --> 放置项目的类路径classpath中即可
~~~
...
<configuration>
  ...
  <mappers>
    <mapper resource="com/codermaast/example/BlogMapper.xml"/>
  </mappers>
</configuration>
```
...
> 这里Mapper文件写SQL语句是有两种方式，==XML文件==和Java的Mapper==类注解==。这里主要对XML方式进行详细记录，因为在实际开发中XML形式能够适应更加复杂的开发需求，且耦合度较低。注解方式开发，虽然能够和接口方法紧密绑定，但实际上SQL语句的编写适应范围不够广泛，仅能满足简单的增删改查操作，且代码的耦合度较高，不利于后期的维护。

XML的SQL中有四种标签：

`<select>`：查询标签，主要书写查询相关的SQL语句，可以指定返回值类型。

`<insert>`：插入标签，可以接受参数，但返回值类型为整型，不能自己指定，返回值为插入成功的行数。

`<update>`：更新标签，可以接受参数，但返回值类型为整型，不能自己指定，返回值为更新成功的行数。

`<delete>`：删除标签，可以接受参数，但返回值类型为整型，不能自己指定，返回值为删除成功的行数。

### XML中构建对象

```java
// 获得核心配置文件
InputeStream resourceAsStream = Resources.getResourceAsStream("sqlMapConfig.xml");
// 获得Session工厂
SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(resourceAsStram);
// 获得Session会话对象
...
> 这里获取到的sqlsession对象默认是事务不提交的，需要我们手动提交。执行sqlsession.commit();方法即可。
>
> 如果想要获取到一个默认提交的sqlsession对象，只需要在sqlsessionFactory对象的opensession方法内传入true参数即可。
>
> 如：SqlSession sqlSession = sqlSessionFactory.openSession(true);	就获得的是默认提交的sqlsession。
...
```xml
<properties resource="com/mybatis/codermast/config.properties">
  <property name="username" value="codermast"/>		<!--将codermast作为值赋值给username变量-->
  <property name="password" value="123456789"/>  	<!--将c123456789作为值赋值给password变量-->
  <property name="url" value="https://www.codermast.com"/>	<!--将https://www.codermast.com作为值赋值给url变量-->
</properties>
```

> 通过`resource`属性进行配置文件的引入，引入以后可以在后面的内容中直接调用。

在这里配置好的属性值，可以在整个配置文件中替换所需要动态变化的值。在使用这些值时，需要使用${变量名}的格式，否则xml文件将其会直接解析为字符串，则无法实现动态替换的效果。

> 例如，我们在配置数据源信息时。就可以使用这种配置信息，这里的${username}就可以是我们之前配置好的用户名。但是这里的
>
> ${driver}我们并没有配置，则它会默认读取resource内的文件的对应信息。也就是config.properties内的对应信息。
...
> 这里就会有读者很疑惑，为什么是最后加载的反而是优先级最高的，不应该是最先加载的才优先级最高吗？这里的加载实际上是覆盖加载，可以理解为最后加载的内容覆盖掉了前面的所有出现的内容，故生效的应该是最后加载的。
...
> 如果你配置了上面的信息，也就是修改了分割符，那么你在后面的使用中，则使用你修改过的分割符进行分割即可。
...
>  Setting标签是MyBatis 中极为重要的调整设置，它们会改变 MyBatis 的运行时行为。 下表描述了设置中各项设置的含义、默认值等。你可以选择性的设置你所想要设置的内容，也不必全部都进行设置。在你没有明确声明的时候，会将对应的标签赋予默认值。
...
> 这是一个完整的Setting标签案例，你可以在你想更改配置的时候，将其复制到你的项目里，也可以对对应的属性值进行修改配置。
...
> 类型别名可为 Java 类型设置一个缩写名字。这样做以后，在后续的XML配置中，可以直接使用别名，而不必使用类的全限定名。 它仅用于 XML 配置，意在降低冗余的全限定类名书写。
...
> 直接对同一个包下的所有类设置别名，你不必去指定每个类的别名，默认的别名就为包下类名的小写名称。例如：类名为User，则其对应的别名就是user。

#### 使用注解设置别名

> 直接在对应的类中使用@Alias("value")注解，可以不必在xml配置文件内配置。设置的value值就是你想设置的别名名称。格式为字符串。
...
>  下面是一些为常见的 Java 类型内建的类型别名。它们都是不区分大小写的，注意，为了应对原始类型的命名重复，采取了特殊的命名风格。对于这些别名，你可以不必再去自定义，默认就可以直接使用别名。
...
设置预处理语句（PreparedStatement）中的参数或从结果集中取出一个值时， 都会用类型处理器将获取到的值以合适的方式转换成 Java 类型。简单的来说就是从数据库获取的数据，将其的数据类型按照这个规则转换成Java中对应的数据类型。

> 从 3.4.5 开始，MyBatis 默认支持 JSR-310（日期和时间 API） 。

那么简单的思考一下，我们为什么要这样做呢？这样做有必要吗？

> 其实答案很显然，因为数据库中的数据类型和Java中的数据类型并不是一一对应的，如果是一一对应的，我们直接就可以在Java内部设置类型转换。而且对于不同的数据库其对应的数据类型也并不是完全一样的，甚至同一厂商的同一数据库的不同版本所对应的数据类型都是不同的。

#### 自定义处理器

​		创建一个自定义的类型处理器有一下两种方式，==实现TypeHandler接口==和==继承BaseTypeHandle类==我们就这两种方法展开学习。

##### 实现TypeHandler接口

​		即然要实现`TypeHandle`接口，那么我们最起码得先知道这个接口在哪里。这个接口位于`org.apache.ibatis.type.TypeHandle`

​		找到了这个接口在哪里，我们的目的是实现这个接口，那么就应该知道实现这个接口应该实现那些方法，这些方法都有什么功能，我们应该如何去实现它。按照这个方法我们很快就能够达成目标。下面的继承`BaseTypeHandle`类也是同样的方法，就不再赘述。希望读者能自己独立的实现。
...
public interface TypeHandler<T> {
    ...
    void setParameter(PreparedStatement var1, int var2, T var3, JdbcType var4) throws SQLException;
		...
		// 数据库类型 ==> java类型	 根据列名称处理
    T getResult(ResultSet var1, String var2) throws SQLException;
  
		// 数据库类型 ==> java类型  根据列号处理
    T getResult(ResultSet var1, int var2) throws SQLException;
  
		// 数据库类型 ==> java类型  针对存储过程转换结果
    T getResult(CallableStatement var1, int var2) throws SQLException;
}
```

实现这四个方法即可，那么具体的实现如何书写呢？先不要着急，我们一步一步来。

##### 继承BaseTypeHandler类

即然要继承`BaseTypeHandle`接口，同理，我们应该知道这个类在哪。这个接口位于`org.apache.ibatis.type.TypeHandle`
...
​		从源码中我们首先能看到，这个类是一个抽象类，则其不能够实例化。

在看到该类继承TypeReference类以后，还实现了我们刚才解读的TypeHandle接口，则这个类也是TypeHandle类的一个实现类。

​		当我们需要继承该类的时候，我们需要实现该类中所有的抽象方法。很显然，该类中虽然有10个方法，但是只有4个抽象方法，所以需要我们实现的方法就只有这四个方法。仔细观察，这四个抽象类就是我们前面看到的TypeHandle接口中的四个接口方法。

​		故这两种方法都能实现自定义一个处理器，但是实际上都只需要实现同样的四个类。也就是这两种方法的本质是相似的。

##### 官方案例

> 这是mybatis官方给定的一个演示案例，我们可以参考该案例来书写我们自己的处理器类。
...
> 值的注意的是，当我们编写完自己的处理器以后，重启项目，发现并没有按照我们配置的处理器进行数据处理，这是因为我们并没有在xml配置文件中配置自定义的处理器，我们需要进行配置以后，我们的自定义处理器才会生效。

#### 默认处理器

> 下面是一些mybatis默认内置的一些类型处理器。这些处理器是不需要我们自己去定义的，mybatis已经内置了。我们直接使用就可以了。
...
<!-- 自定义类型处理器 -->
<typeHandlers>
    <typeHandler handler="com.codermast.handler.DateTypeHandler"/>
</typeHandlers>
```

> handler属性的值即为我们自定义处理器的全限定名。也就是类的地址。
...
<!-- mybatis-config.xml -->
<typeHandlers>
  <package name="com.codermast.handle"/>
</typeHandlers>
```
...
实际上对象工厂我们已经在前面已经见到过了，没有印象的读者可以返回到第三小节-配置文件，那里我们有一个获取sqlsession的工厂对象。
...
```xml
<!-- mybatis-config.xml -->
<objectFactory type="com.codermast.objectFactory.ExampleObjectFactory">
...
这些配置会告诉 MyBatis 去哪里找映射文件，剩下的细节就应该是每个 SQL 映射文件，也就是我们需要实现的SQL部分。

> 学完了项目的配置，那么接下来我们应该把重点放到我们需要实现的数据层操作了，也就是直接对数据的操作了。在Mybatis中我们是通过编写mapper.xml文件来进行对数据的操作的。
...
> 这里并没有对插入、更新、删除操作单独进行分析。主要的原因是这三者的处理逻辑是比较类似的，三者的操作顺序、次数等因素都会对数据的读取产生影响。这一点和查询不同，故在此奖此三者归为一类进行分析。

数据变更语句 insert，update 和 delete 的实现非常接近，主要还是关心其标签内的属性功能。接下来就对这三者的属性进行描述分析。
...
> 这里不推荐使用注解，即便在类中写注解会看起来很方便，在项目的体量比较小，对安全的要求也不高的情况下，会提高效率，但是对于项目的维护会产生巨大的阻碍，增大了项目的耦合度，致使在项目的后期更新中需要大量改写注解，甚至出现注解无法满足需求的情况出现，故最好在一开始直接使用xml进行配置。
...
> 做一件事必须要有始有终，很多时候我们会安装，但并不会卸载，这里就简单的说一下卸载。即便很简单，但我相信还是有很多读者不太懂，这里就多说一些。

- 删除maven中的依赖

> 这种方法很简单，直接删掉maven中的依赖项即可。也就是删掉下面的这段代码。
...
> 开始时在项目中引入的jar包，在对应的路径删掉这个包文件即可。

## 写在最后

本文的书写尽管只用了两天时间，但是实际内容的理解也花费了我很久的时间，从接触的mybatis这门技术，我也是从看视频，看文档，然后自己实现，自己编写这个阶段慢慢走过来的。

很多读者找到我，其中不乏大量的初学者。他们告诉我，那些大佬们好像比我们更聪明，他们和我们在学习同种从未接触的新东西时，他们的学习能力和理解能力好不夸张的来说比我们高不止一个维度，可能我们需要花费数月的时间学习的东西，他们几天就学会了，不仅仅学的快，而且更要命的是他们使用这些技术看起来更加的娴熟。

其实这种现象是很正常不过的，简单的分析一下，他们真的比你更聪明么，其实也不见得。我觉得最重要的一点是，他们拥有项目的实际上手操作经验，对于一个技术点能从不同的角度去思考，反过来再看看你自己本身的学习，仅仅只停留在技术的本身，并未思考如何去应用，在不同的场景下如何去选择。长久以来，差距会越来越大，良性循环和恶性循环的差距会随着时间的改变越来越大，起初你可能只是觉得他就比你见得多了而已，以后你把业务场景再补一补就好了，实际上这种思想是大错特错的。

这种降维打击式的学习，能在学习的过程中极大的掌握知识。每学一个点，他们就能联想到自己曾经碰到的业务场景，如果使用这种技术是否会更加的高效、便捷。不知不觉中也对业务场景的应用有了简单的掌握，下次碰到这种情况时，最起码能够想到该如何去选择实现方式，对于项目的整体掌控是对技术高效学习的制胜法宝。

> 本文参考了大量来自于[MyBatis中文网](https://mybatis.net.cn/index.html)的文档教程，在此表示感谢。
>
> 另外我还创建了一个知识星球，永久免费，里面分享很多关于计算机相关知识的信息，无论你是大一刚入学的小萌新，还是大四才醒悟想找工作的老学长。又或者是打算计算机考研，你在这里都能够获得到你想要的信息。
>
> 如果你目前都没有这方面的打算，你也可以进来看看。凑凑热闹，看看大家都是如何准备的，有些时候的有些东西你是毫无意识的错过，多看看学长学姐的考研分享，或者想了解一些职场上的技巧经验，你都可以加入，或许你在这里并不能收获到很多专业性的知识和技术，但你可能因此确定了人生的努力方向，这亦是本星球带给你最大的收获。
>
> 欢迎你的加入💝！我们在这里等你。也欢迎大家关注我的个人公众号 ==爱编程的mast==，会分享一些我在学习生活中我觉得有意义的事情，也希望能带给你们帮助。谢谢！

![公众号-爱编程的mast](images/%E5%85%AC%E4%BC%97%E5%8F%B7-%E7%88%B1%E7%BC%96%E7%A8%8B%E7%9A%84mast-20221125105425862.png)

![知识星球推广](images/%E7%9F%A5%E8%AF%86%E6%98%9F%E7%90%83%E6%8E%A8%E5%B9%BF.jpeg)
